...
//Graphs a note at x for a staff at y given its n_value (calculated with its letter and number), lengthDivisor (e.g. 4 for a quarter note), 
...
//Graphs an accidental at x for a staff at y given its value (calculated the same as n_value), .eps name, and the current clef.
bool graphAccidental(const double &x, const double &y, const int &value, const string &img, const int &clef);

//Graphs a time signature at x for a staff at y given its upper and lower values.
void graphTimeSignature(const double &x, const double &y, const double &top, const int &bot);

//Graphs a key signature at x for a staff at y given the number of accidentals; whether they are sharps, flats, or naturals (represented by 's', 'b', and 'n',
...
//Graphs the end bar at the given x for a staff at y
